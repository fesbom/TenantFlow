...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/reset-password", async (req, res) => {
    try {
      ...
      console.log(
        `Password reset requested for: ${email}${user ? " (user found)" : " (user not found)"}`,
      );
  ...
  app.post("/api/auth/register-clinic", async (req, res) => {
    try {
      const { clinicName, clinicEmail, adminName, adminEmail, password } =
        req.body;
  ...
  app.get(
    "/api/dashboard/birthday-patients",
    authenticateToken,
    async (req: AuthenticatedRequest, res) => {
      try {
        const page = parseInt(req.query.page as string) || 1;
        const pageSize = parseInt(req.query.pageSize as string) || 5;
        const paginatedResult = await storage.getBirthdayPatients(
          req.user!.clinicId,
          { page, pageSize },
        );
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (error.name === "ZodError") {
          ...
          return res
            .status(400)
            .json({ message: `Dados inválidos: ${fieldErrors}` });
        }
        res
          .status(500)
  ...
  app.put(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const updatedClinic = await storage.updateClinic(
          req.user!.clinicId,
          clinicData,
        );
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (req.file.size > maxSize) {
          return res
            .status(400)
            .json({ message: "O arquivo é muito grande. Tamanho máximo: 5MB" });
        }
        ...
        if (
          !/^[a-zA-Z0-9_-]+$/.test(clinicId) ||
          !/^[a-zA-Z0-9_-]+$/.test(patientId)
        ) {
        ...
        if (patient.photoUrl) {
          if (
            patient.photoUrl.startsWith("gcs://") ||
            ...
            try {
              ...
              console.error(
                "Error deleting old photo from filesystem:",
                deleteError,
              );
            }
          }
  ...
  app.get(
    "/api/patients/by-external-id/:externalId",
    authenticateToken,
    async (req: AuthenticatedRequest, res) => {
      try {
        const cleanedExternalId = String(parseInt(req.params.externalId, 10));
        const patient = await storage.getPatientByExternalId(
          cleanedExternalId,
          req.user!.clinicId,
        );
        if (!patient) {
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (updateData.photoUrl === null) {
          const currentPatient = await storage.getPatientById(
            req.params.id,
            req.user!.clinicId,
          );
          if (currentPatient?.photoUrl) {
            if (
              currentPatient.photoUrl.startsWith("gcs://") ||
              currentPatient.photoUrl.startsWith("http")
            ) {
              try {
                const objectStorageService = new ObjectStorageService();
              ...
              try {
                const filePath = path.join(
                  process.cwd(),
                  currentPatient.photoUrl.replace(/^\//, ""),
                );
                ...
                console.error(
                  "Error deleting photo from filesystem:",
                  deleteError,
                );
              }
            }
  ...
  app.delete(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        const deleted = await storage.deletePatient(
          req.params.id,
          req.user!.clinicId,
        );
        if (!deleted) {
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        const appointments = await storage.getAppointmentsByClinic(
          req.user!.clinicId,
        );
        res.json(appointments);
      } catch (error) {
      ...
      try {
        ...
        if (updateData.duration !== undefined) {
          ...
          const scheduledDate =
            ...
            (
              await storage.getAppointmentById(
                req.params.id,
                req.user!.clinicId,
              )
            )?.scheduledDate;
  ...
  app.delete(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        const deleted = await storage.deleteAppointment(
          req.params.id,
          req.user!.clinicId,
        );
        if (!deleted) {
          ...
          if (isProduction) {
            ...
            for (const file of files) {
              try {
                ...
                console.error(
                  "Error uploading medical record image:",
                  uploadError,
                );
              }
            }
            ...
            for (const file of files) {
              ...
              imagePaths.push(
                `/uploads/${clinicId}/medical-records/${patientId}/${tempRecordId}/${filename}`,
              );
            }
          }
        }
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        const questions = await storage.getAnamnesisQuestionsByClinic(
          req.user!.clinicId,
        );
        res.json(questions);
      } catch (error) {
      ...
      try {
        ...
        for (const responseData of responses) {
          if (responseData.response && responseData.response.trim()) {
            const response =
              await storage.createAnamnesisResponse(responseData);
  ...
  app.get(
    "/api/anamnesis/treatment/:treatmentId",
    authenticateToken,
    async (req: AuthenticatedRequest, res) => {
      try {
        const { treatmentId } = req.params;
        const anamnesis = await storage.getAnamnesisWithResponsesByTreatment(
          treatmentId,
          req.user!.clinicId,
        );
        res.json(anamnesis);
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const authReq = req as AuthenticatedRequest;
        const treatmentsData = await storage.getTreatmentsByPatient(
          patientId,
  ...
  app.get("/api/treatments/:id", authenticateToken, async (req, res) => {
    try {
      const { id } = req.params;
      const authReq = req as AuthenticatedRequest;
      const treatment = await storage.getTreatmentById(
        id,
        authReq.user!.clinicId,
      );
      if (!treatment) {
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        const { id } = req.params;
        const authReq = req as AuthenticatedRequest;
        const deleted = await storage.deleteTreatment(
          id,
          authReq.user!.clinicId,
        );
        if (!deleted) {
  ...
  app.get(
    "/api/budget-items/treatment/:treatmentId",
    authenticateToken,
    async (req, res) => {
      try {
        const { treatmentId } = req.params;
  ...
  app.get(
    "/api/budget-summary/treatment/:treatmentId",
    authenticateToken,
    async (req, res) => {
      try {
        const { treatmentId } = req.params;
        const budgetSummary =
          await storage.getBudgetSummaryByTreatment(treatmentId);
        res.json(budgetSummary);
      } catch (error) {
  ...
  app.get(
    "/api/treatment-movements/treatment/:treatmentId",
    authenticateToken,
    async (req, res) => {
      try {
        const { treatmentId } = req.params;
        const movements =
          await storage.getTreatmentMovementsByTreatment(treatmentId);
        res.json(movements);
      } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        if (!currentMovement) {
          return res
            .status(404)
            .json({ message: "Treatment movement not found" });
        }

        const movementData = { ...req.body };
        delete movementData.removePhoto;
        const result = insertTreatmentMovementSchema
          .partial()
        ...
        const removePhoto =
          String(req.body.removePhoto).toLowerCase() === "true";

        if (req.file) {
          if (currentMovement.fotoAtividade) {
            if (
              currentMovement.fotoAtividade.startsWith("gcs://") ||
              currentMovement.fotoAtividade.startsWith("http")
            ) {
              try {
                const objectStorageService = new ObjectStorageService();
              ...
              try {
                const filePath = path.join(
                  process.cwd(),
                  currentMovement.fotoAtividade.replace(/^\//, ""),
                );
                if (fs.existsSync(filePath)) {
                  fs.unlinkSync(filePath);
                }
        ...
        const movement = await storage.updateTreatmentMovement(
          id,
          validatedData,
        );
        if (!movement) {
  ...
  app.get(
    "/api/anamnesis-responses/treatment/:treatmentId",
    authenticateToken,
    async (req, res) => {
      try {
        const { treatmentId } = req.params;
        const responses =
          await storage.getAnamnesisResponsesByTreatment(treatmentId);
        res.json(responses);
      } catch (error) {
  ...
  app.post(
    "/api/anamnesis-responses/treatment",
    authenticateToken,
    async (req, res) => {
      try {
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      ...
      async function findTreatmentByOldId(
        oldId: string,
      ): Promise<string | null> {
        try {
      ...
      try {
        ...
        if (!type)
          return res
            .status(400)
            .json({ success: false, message: "Import type is required" });
        ...
        switch (type) {
            ...
            for (const row of csvData) {
              try {
                ...
                const patientData = {
                  ...
                  responsibleDentistId: row.cd_dentista_original
                    ? idMapping.get(`user_${row.cd_dentista_original}`) || null
                    : null,
                  ...
                  lastVisitDate: row.dt_ultima_visita
                    ? parseDate(row.dt_ultima_visita)
                    : null,
                  lastContactDate: row.dt_ultimo_contato
                    ? parseDate(row.dt_ultimo_contato)
                    : null,
                  clinicId: req.user!.clinicId,
                  externalId: row.cd_paciente
                    ? row.cd_paciente.toString()
                    : null,
                };
                if (!patientData.fullName)
                  throw new Error("Missing nm_paciente");
                const patient = await storage.createPatient(patientData);
                if (row.cd_paciente)
                ...
                errors.push(
                  `Row ${csvData.indexOf(row) + 1}: ${error.message}`,
                );
                failed++;
              }
            }
          ...
          case "treatments":
            for (const row of csvData) {
              try {
                ...
                const patientId =
                  idMapping.get(`patient_${row.cd_paciente}`) ||
                  (await findPatientByOldId(row.cd_paciente));
                if (!patientId)
                  throw new Error(`Patient ${row.cd_paciente} not found`);
                const treatmentData = {
                  patientId,
                  dentistId: req.user!.id,
                  clinicId: req.user!.clinicId,
                  ...
                  tituloTratamento:
                    row.ds_obs?.trim() || "Tratamento sem descrição",
                  externalId: row.cd_tratamento
                    ? row.cd_tratamento.toString()
                    : null,
                };
                const treatment = await storage.createTreatment(treatmentData);
                if (row.cd_tratamento)
          ...
          case "budget-items":
            for (const row of csvData) {
              try {
                const compositeExternalId =
                  row.cd_tratamento && row.sequencial
                    ? `${row.cd_tratamento}-${row.sequencial}`
                    : null;
                ...
                const treatmentId =
                  idMapping.get(`treatment_${row.cd_tratamento}`) ||
                  (await findTreatmentByOldId(row.cd_tratamento));
                if (!treatmentId)
                  throw new Error(`Treatment ${row.cd_tratamento} not found`);
        ...
        res.json({
          success: true,
          summary: { totalRows: csvData.length, imported, skipped, failed },
  ...
  app.post("/webhook/zapi", async (req, res) => {
    ...
    if (ZAPI_CLIENT_TOKEN) {
      ...
      if (clientToken !== ZAPI_CLIENT_TOKEN) {
        console.warn(
          "⚠️ [Z-API Webhook] Invalid or missing Client-Token header",
        );
    ...
    try {
      ...
      if (!phone || !messageText) {
        console.warn(
          "⚠️ [DEBUG] Campos obrigatórios ausentes (phone ou messageText).",
        );
        return res
          .status(200)
          .json({ received: true, warning: "Missing required fields" });
      }

      const normalizedPhone = phone.replace(/\D/g, "");
      console.log(
        `📱 [DEBUG] Paciente: ${normalizedPhone} | Mensagem: "${messageText}"`,
      );
      ...
      if (!conversation) {
        console.log(
          "🆕 [DEBUG] Conversa não encontrada. Criando nova conversa...",
        );
      ...
      console.log(
        `✅ [DEBUG] ID da Conversa no Banco: ${conversation.id} | Status: ${conversation.status}`,
      );
      ...
      if (conversation.status === "ai") {
        console.log(
          "🤖 [DEBUG] Modo IA ativo. Buscando histórico e chamando Gemini...",
        );

        const messages = await storage.getWhatsappMessagesByConversation(
          conversation.id,
        );
        ...
        if (aiResponse.extractedIntent.intent === "falar_com_humano") {
          console.log(
            "👤 [DEBUG] Intenção de falar com humano detectada. Alterando status...",
          );
        ...
        if (!sendResult.success) {
          console.error(
            "❌ [DEBUG] Falha ao enviar resposta via Evolution:",
            sendResult.error,
          );
        }
      } else {
      ...
      try {
        ...
        if (evolutionMessageId) {
          const existingMessage =
            await storage.getWhatsappMessageByExternalId(evolutionMessageId);
          if (existingMessage) {
            console.log(
              `[WEBHOOK] Mensagem duplicada ignorada: ${evolutionMessageId}`,
            );
            return;
          }
        }
        ...
        if (conversation.status === "ai") {
          ...
          if (aiResponse.extractedIntent.intent === "agendar") {
            ...
            if (date && time && dentistNameRaw) {
              ...
              const cleanName = dentistNameRaw
                .replace(/Dr\.|Dra\.|doutor|doutora/gi, "")
                .trim();
              const dentist = allUsers.find(
                (u) =>
                  u.role === "dentist" &&
                  u.fullName.toLowerCase().includes(cleanName.toLowerCase()),
              );
              ...
              if (dentist) {
                ...
                if (isBusy) {
                  ...
                  console.log(
                    `[SUCESSO] Agendamento criado para ${dentist.fullName} em ${date} às ${time}`,
                  );
                  aiResponse.message = `Perfeito! Seu agendamento com o ${dentist.fullName} foi confirmado para o dia ${date} às ${time}.`;
                }
              } else {
                console.log(`[ERRO] Dentista não encontrado: ${cleanName}`);
              }
            }
          }
  ...
  app.get("/admin/setup-whatsapp", async (req, res) => {
    ...
    try {
      ...
      console.log(
        `📊 [Evolution] Primeiros 50 chars: ${qrBase64.substring(0, 50)}...`,
      );
  ...
  app.get("/admin/set-webhook", async (req, res) => {
    ...
    if (WEBHOOK_GLOBAL_URL) {
      ...
      if (host && !host.includes("localhost")) {
        ...
        console.log(
          `⚠️ [Set Webhook] WEBHOOK_GLOBAL_URL não configurado. Usando headers: ${host}`,
        );
      } else {
        console.error(
          "❌ [Set Webhook] Não foi possível determinar a URL do webhook",
        );
        return res.status(400).json({
          success: false,
          error:
            "Configure WEBHOOK_GLOBAL_URL nos Secrets ou acesse via URL pública do Replit",
        });
      }
    }
    ...
    console.log(
      `🌐 [Set Webhook] Configurando Webhook na URL REAL: ${webhookUrl}`,
    );
    console.log("========================================\n");
    ...
    try {
      ...
      console.error(
        `❌ [Set Webhook] Erro ao configurar webhook:`,
        error.message,
      );
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        const conversationsList =
          await storage.getWhatsappConversationsByClinic(req.user!.clinicId);
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const messagesList =
          await storage.getWhatsappMessagesByConversation(id);
